...
class PDMotionPlanner(Node):
    ...
    def control_loop(self):
        ...
        # Linear error is the x-distance to target in robot frame
        linear_error = next_pose_robot_tf[0][3]
        
        # Angular error is the heading difference - extract yaw angle from transformation
        # Using arctan2 on the rotation matrix elements
        angular_error = math.atan2(next_pose_robot_tf[1][0], next_pose_robot_tf[0][0])
        
        dt = (self.get_clock().now() - self.last_cycle_time).nanoseconds / 1e9
