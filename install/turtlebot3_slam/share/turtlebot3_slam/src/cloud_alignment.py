...
class ScanAlightment:
    ...
    def create_point_cloud(self, beam_lenght):
        ...
        for r in beam_lenght:
            if self.range_min <= r < self.range_max:
                ...
                transformed_point = self.Trs(Vector2D(px, py))
                if isinstance(transformed_point, Vector2D):
                    points.append([transformed_point.x, transformed_point.y, 0.0])
                else:
                    points.append([transformed_point[0], transformed_point[1], 0.0])
    ...
    def pcl_icp_wrapper(self, T: Transform2D, T_init, beam_length):
        if self.first_scan_receive:
            ...
            T.v2.x, T.v2.y, T.theta = T_new.v2.x, T_new.v2.y, T_new.theta
    ...
    def pcl_icp(self, T_init: Transform2D, 
                target: o3d.geometry.PointCloud,
                source: o3d.geometry.PointCloud)->Tuple[bool, Transform2D]:
        ...
        return True, Transform2D(Vector2D(tx, ty), theta)  # Fixed constructor call
